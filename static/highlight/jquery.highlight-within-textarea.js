	...
	HighlightWithinTextarea.prototype = {
		...
		getCustomRanges: function(input, custom) {
			...
			if (custom.color) {
				ranges.forEach(function(range) {
					...
					range.color = custom.color;
					range.subject = custom.subject;
				});
			}
			return ranges;
		},
		...
		renderMarks: function(boundaries) {
			...
			input = input.replace(/\{\{hwt-mark-start\|(\d+)\}\}/g, function(match, submatch) {
				...
				var color = boundaries[+submatch].color;
				var subject = boundaries[+submatch].subject;
				if (color) {
					return `<mark data-subject="${subject}" style="background-color: ${color}">`;
				} else {
					return `<mark data-subject="${subject}" style="background-color: red">`;
				}
			});
